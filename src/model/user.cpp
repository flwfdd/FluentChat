...
UserModel::UserModel(int id, const QString &username, const QString &nickname, const QColor &color, const QString &avatar, const QString &remark, bool online, QObject *parent)
    : QObject(parent), m_id(id), m_username(username), m_nickname(nickname), m_color(color), m_avatar(avatar), m_remark(remark), m_online(online)
{
}
...
QColor UserModel::color() const
{
    return m_color;
}

void UserModel::setColor(const QColor &color)
{
    if (m_color != color) {
        m_color = color;
        emit colorChanged();
